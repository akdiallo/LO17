...
public class LanceRequete extends HttpServlet {
	...
	public void corpusTests() throws IOException {
		BufferedReader br = new BufferedReader(new FileReader("/Users/Romain/Dropbox/UTC/GI04/LO17/TP/TD4/corpusQuestionSortedUniq.txt"));
		String line;
		while ((line = br.readLine()) != null) {
			System.out.println("\n"+line);
			TalMain3 grammaire = new TalMain3(line);
			...
			try {
			    ...
			    System.out.println(rowcount+" resultats");
			} catch(Exception e){}
		}
		br.close();
	}
   ...
   //     url = "jdbc:postgresql://tuxa/dblo17";
	...
	if (requete_nl != null) {
		...
		if (requete.length() < 3) {
	        out.println("<span class='error'> La requête a retourné 0 resultat </span>");
		}
		
		out.println("<table id='info-table'><tr><td class='table-th'>Reqête NL:</td><td>"+requete_nl+"</td></tr>");
		...
		out.println("<tr><td class='table-th'>Requête SQL: </td><td>"+requete+"</td></tr></table>");
        out.println("</div>");
		...
		try {
	        ...
	        out.println("<div class='info'>");
		    out.println("<span class='no-error'> La requête a retourné "+rowcount+" résultats</span>");
	        out.println("</div>");
